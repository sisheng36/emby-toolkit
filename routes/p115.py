...
def local_organize_sorting_rules():
    key = 'local_organize_sorting_rules'          # ★ 与任务代码一致
    if request.method == 'GET':
        raw = settings_db.get_setting(key)
        ...
        for r in rules:
            if 'id' not in r:
                r['id'] = str(int(time.time() * 1000))
        return jsonify({"success": True, "data": rules})   # ★ 统一返回格式

    # POST 保存
    rules = request.json
    if not isinstance(rules, list):
        rules = []
    settings_db.save_setting(key, rules)
...
def local_organize_rename_config():
    key = 'local_organize_rename_config'          # ★ 与任务代码一致
    if request.method == 'GET':
    ...
    # POST 保存
    settings_db.save_setting(key, request.json)
    return jsonify({"success": True, "message": "重命名规则已保存"})
